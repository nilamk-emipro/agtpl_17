...
const { useDebugCategory } = require("@web/core/debug/debug_context");
...
const { CallbackRecorder } = require("@web/webclient/actions/action_hook");
...
__exports.clearUncommittedChanges = clearUncommittedChanges; function clearUncommittedChanges(env) {
    const callbacks = [];
...
function makeActionManager(env) {
    ...
    async function _printPdf(url) {
        return new Promise(function(resolve, reject) {
            ...
            myiFrame.addEventListener("load", function(){
                let element_inside_iframe = myiFrame.contentWindow
    ...
    async function _executeReportAction(action, options) {
        ...
        if (action.report_type === "qweb-html") {
            ...
            const context = encodeURIComponent(JSON.stringify(env.services.user.context));
            url += `?context=${context}`;
            return _printPdf(url).then(function(iframe){
                iframe.focus();
                iframe.print();
            }).catch(function(e){
                 console.log('error=' + e)
                 _triggerDownload(action, options, 'pdf');
            }).finally(function(){
                framework.unblockUI();
            });
           // }
...
const actionService = __exports.actionService = {
